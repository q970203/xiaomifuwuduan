...
//链接库
let open = ({ dbName='xiaomi', collectionName, url = 'mongodb://127.0.0.1:27017' }) => {
...
let findList = ({
  collectionName,//集合名
  dbName = 'xiaomi',//库名 默认值
  _page, _limit, _sort, q ,_type//可选参数
}) => {
  //_sort 排序    q 检索   _page 页数  _limit:限定条数
  //生成检索条件，给collectoin.find(条件,{配置})方法 使用 
  // let rule = q ? { username: new RegExp(q,g) } : {}
  let rule1 = q ? { title: eval('/' + q + '/') } : {}
	let rule2 = _type ? {type: eval('/'+ _type +'/')}:{}
  return new Promise((resolve, reject) => {
    //链库
    open({
