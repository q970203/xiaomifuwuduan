...
router.post('/', (req, res, next) => {
  ...
  //抓取参数
  let { title,newj,oldj,des,type,opt1,opt2,opt3,color1,color2,color3} = req.body;

  let img1,img2,img3;//保存多个图片地址（网络）
...
//     }
    if (file.fieldname === 'img1'|'img2'|'img3') {
      banner = '/upload/miaosha/' + file.filename + pathLib.parse(file.originalname).ext;
    }
  ...
  mgdb.open({
    ...
    ({ collection, client }) => {
      ...
      let data = { title, banner, time, des,type }
